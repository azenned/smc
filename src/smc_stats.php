<?php
require_once __DIR__ . '/Cache/IPCHelper.php';

use \Blueflame\Cache\IPCHelper;

////////// BEGIN OF DEFAULT CONFIG AREA ///////////////////////////////////////////////////////////

defaults('SMC_DASHBOARD_RESTRICT', 0);      // Use (internal) authentication - best choice if
// no other authentication is available
// If set to 0:
//  There will be no further authentication. You
//  will have to handle this by yourself!
// If set to 1:
//  You need to change SMC_DASHBOARD_PASSWORD to make
//  this work!
defaults('SMC_DASHBOARD_USER', 'smc');        // Admin Username
defaults('SMC_DASHBOARD_PASSWORD', 'password');    // Admin Password - CHANGE THIS TO ENABLE!!!
...
if (empty($MYREQUEST['ID']) or $MYREQUEST['ID'] == 0) {
  $MYREQUEST['ID'] = "apcu";
}
...
if (empty($MYREQUEST['PG'])) {
  $MYREQUEST['PG'] = 0;
}
if (!isset($MYREQUEST['COUNT']) or ($MYREQUEST['COUNT'] == 0)) {
  $MYREQUEST['COUNT'] = 20;
}
...
$ALL_INSTANCES = (IPCHelper::__getAllInstancesStatus() ?? []);
$ALL_INSTANCES_IDS = array_keys($ALL_INSTANCES);
if (count($ALL_INSTANCES)) {
  if (isset($ALL_INSTANCES[(int) $MYREQUEST['ID']]) && (0 !== (int) ($MYREQUEST['ID']))) {
    $info = $ALL_INSTANCES[(int) $MYREQUEST['ID']];
  }
  else {
    $info = current($ALL_INSTANCES);
    $MYREQUEST['ID'] = $info['key'];
  }

  if (!$info['access'] ?? FALSE) {
    die('Permission denied to acces this SHM memory');
  }

  try {
    $INSTANCE = new \Blueflame\Cache\SHMStorage((int) $info['key'], FALSE);
  } catch (\Exception $e) {
    die($e->getMessage());
  }
  ...
  $INSTANCE = new \Blueflame\Cache\SHMStorage('apcu', FALSE);
}
...
//
if (!SMC_DASHBOARD_RESTRICT) {
  $AUTHENTICATED = 1;
}
else {
  $AUTHENTICATED = 0;
  if (SMC_DASHBOARD_PASSWORD != 'password' && ($MYREQUEST['LO'] == 1 || isset($_SERVER['PHP_AUTH_USER']))) {

    if (!isset($_SERVER['PHP_AUTH_USER']) ||
      !isset($_SERVER['PHP_AUTH_PW']) ||
      $_SERVER['PHP_AUTH_USER'] != SMC_DASHBOARD_USER ||
      $_SERVER['PHP_AUTH_PW'] != SMC_DASHBOARD_PASSWORD) {
      Header("WWW-Authenticate: Basic realm=\"SMC Login\"");
...
if ($AUTHENTICATED && !empty($MYREQUEST['DU'])) {
  $INSTANCE->delete($MYREQUEST['DU']);
}

if (!class_exists('Blueflame\Cache\SHMStorage')) {
  echo "No cache info available.  SMC does not appear to be running.";
  exit;
}
$pager_size = (int) $MYREQUEST['COUNT'] ?? 100;
$start = $MYREQUEST['PG'] * $pager_size;

$cache = $INSTANCE->getCacheInfo($MYREQUEST['OB'] != OB_USER_CACHE, $start, $pager_size, FALSE);

$mem = $INSTANCE->getSMAInfo();
...
// create menu entry
function menu_entry($id, $ob, $title) {
  global $MYREQUEST, $PHP_SELF, $ALL_INSTANCES;
  $BASE =
    "$PHP_SELF" .
    "?SCOPE=" . $MYREQUEST['SCOPE'] .
    "&ID=" . $id .
  ...
  if ($MYREQUEST['OB'] != $ob or ($MYREQUEST['ID'] != $id)) {
    return "<li><a href=\"$BASE&OB=$ob\">$title</a></li>";
  }
  else {
    if (empty($MYREQUEST['SH'])) {
      ...
      return "<li><a class=\"child_active\" href=\"$BASE&OB=$ob\">$title</a></li>";
    }
  }
}
...
function put_login_link($s = "Login") {
  ...
  // needs SMC_DASHBOARD_PASSWORD to be changed!
  //
  if (!SMC_DASHBOARD_RESTRICT) {
    return;
  }
  else {
    if (SMC_DASHBOARD_PASSWORD == 'password') {
      print <<<EOB
			<a href="#" onClick="javascript:alert('You need to set a password at the top of smc_stats.php before this will work!');return false";>$s</a>
EOB;
    }
    else {
      if ($AUTHENTICATED) {
...
<html>
<head><title>SMC Cache INFO <?php echo $host ?></title>
  <style><!--
...
<div class="head">
  <h1 class="apc">
    <div class="logo"><span class="logo"><a href="https://github.com/azenned/smc">SMC</a></span></div>
    <div class="nameinfo">SHM based User Cache</div>
  </h1>
  <div class="login">
    <?php put_login_link(); ?>
  </div>
...
EOB;
global $ALL_INSTANCES_IDS, $MYREQUEST;
foreach ($ALL_INSTANCES_IDS as $id) {
  if ($MYREQUEST['ID'] !== $id) {
    echo "<option value='$id' "
      . ($MYREQUEST['ID'] == $id ? " selected " : "")
      . (!($ALL_INSTANCES[$id]['access'] ?? FALSE) ? " disabled " : "")
      . " >$id"
      . (!($ALL_INSTANCES[$id]['access'] ?? FALSE) ? " (Access Denied) " : "")
      . "</option>";
  }
}
echo <<<EOB
...
EOB;
echo
menu_entry($MYREQUEST['ID'], OB_HOST_STATS, 'View Host Stats'),
menu_entry($MYREQUEST['ID'], OB_USER_CACHE, 'User Cache Entries');
...
switch ($MYREQUEST['OB']) {
  ...
  case OB_HOST_STATS:
    ...
    $smc_version = "SMC (APCU compatiblity layer)";
    $phpversion = phpversion();
    $infos = $ALL_INSTANCES[$MYREQUEST['ID']] ?? [];
    $user_info = ($infos['user']['name']) ?? 'unknown';
    $group_info = ($infos['group']['name']) ?? 'unknown';
    $number_vars = $cache['num_entries'];
    $instance_id = $cache['id'];
    $instance_key = $cache['key'];
    ...
    echo <<< EOB
		...
		<tr class=tr-0><td class=td-0>SMC Version</td><td>$smc_version</td></tr>
		<tr class=tr-0><td class=td-0>CACHE Instance ID</td><td>$instance_id</td></tr>
		<tr class=tr-0><td class=td-0>CACHE Instance KEY</td><td>$instance_key</td></tr>
		<tr class=tr-0><td class=td-0>User Owner</td><td>$user_info</td></tr>
		<tr class=tr-0><td class=td-0>Group Owner</td><td>$group_info</td></tr>
		<tr class=tr-1><td class=td-0>PHP Version</td><td>$phpversion</td></tr>
EOB;

    if (!empty($_SERVER['SERVER_NAME'])) {
      echo "<tr class=tr-0><td class=td-0>SMC Host</td><td>{$_SERVER['SERVER_NAME']} $host</td></tr>\n";
    }
...
EOB;
    ...
    graphics_avail() ?
      '<tr>' .
      "<td class=td-0><img alt=\"\" $size src=\"$PHP_SELF?IMG=1&ID=" . $MYREQUEST['ID'] . "&$time\"></td>" .
      "<td class=td-1><img alt=\"\" $size src=\"$PHP_SELF?IMG=2&ID=" . $MYREQUEST['ID'] . "&$time\"></td></tr>\n"
      : "",
    '<tr>',
...
EOB;
    ...
    if (graphics_avail()) {
      ...
      $ID = $MYREQUEST["ID"];
      echo <<<EOB
			<img alt="" $size src="$PHP_SELF?IMG=3&ID=$ID&$time">
EOB;
    }
    echo <<<EOB
...
EOB;
  ...
  case OB_USER_CACHE:
    ...
    echo <<<EOB
		...
		<input type=hidden name=ID value={$MYREQUEST['ID']}>
		<select name=SCOPE>
EOB;
    echo
    ...
    if ($list) {
      ...
      foreach ($list as $k => $entry) {
        if (empty($MYREQUEST['SEARCH_REGEX']) || preg_match($MYREQUEST['SEARCH_REGEX'], $entry[$fieldname]) != 0) {
          ...
          if ($sh == $MYREQUEST["SH"]) {
            echo '<tr>';
            echo '<td colspan="7"><a class="entry-info" href="' . $href . '"><pre>' . htmlentities(print_r($INSTANCE->get($entry['info']), 1)) . '</pre></a></td>';
            echo '</tr>';
          }
          $i++;
...
EOB;
    ...
    // Print pager
    $pager_max = intdiv($cache['num_entries'], $pager_size);
    echo "<ol class=menu>";
    echo "<li><a class='pager' href=\"$MY_SELF&OB=", $MYREQUEST['OB'], "&PG=0\"><i>First</i></a></li>";
    if (($MYREQUEST['PG'] > 0) and ($cache['num_entries'] > $pager_size)) {
      echo "<li><a class='pager' href=\"$MY_SELF&OB=", $MYREQUEST['OB'], "&PG=", $MYREQUEST['PG'] - 1, "\"><i>Previous</i></a></li>";
    }
    echo "<li><a class='pager' href=\"$MY_SELF&OB=", $MYREQUEST['OB'], "&PG=0\"><i>1</i></a></li>";

    // Generate links ids
    $first_range = range(1, min($pager_max, 5));
    $central_range = range(max(0, $MYREQUEST['PG'] - 5), min($pager_max, $MYREQUEST['PG'] + 5));
    $last_range = range(min($pager_max - 1, $pager_max - 5), $pager_max - 1);
    $list = array_merge($first_range, $central_range, $last_range);
    $list = array_filter(array_unique($list, SORT_REGULAR));
    $list = array_diff($list, [0, 1, $pager_max, -5, -4, -3, -2, -1]);
    foreach ($list as $p) {
      echo "<li><a class='pager' href=\"$MY_SELF&OB=", $MYREQUEST['OB'], "&PG=", $p, "\"><i>", $p, "</i></a></li>";
    }
    // Last page
    if ($pager_max > 1) {
      echo "<li><a class='pager' href=\"$MY_SELF&OB=", $MYREQUEST['OB'], "&PG=", $pager_max, "\"><i>", $pager_max, "</i></a></li>";
    }
    if (($MYREQUEST['PG'] < $pager_max) and $cache['num_entries'] > $pager_size) {
      echo "<li><a class='pager' href=\"$MY_SELF&OB=", $MYREQUEST['OB'], "&PG=", $MYREQUEST['PG'] + 1, "\"><i>Next</i></a></li>";
    }
    echo "<ol>";
    echo <<< EOB
		</div>
